...
class NAAS(object):
    ...
    def init_population(self, num_pop, num_layers):
        ...
        while count < num_pop:
            ...
            reward, constraint = self.exterior_search(0, self.model_defs[0], rand_gene, sample_map_gene)
            if not(reward==None and (constraint==None or constraint > self.constraint_value)): #select valid gene. Incorrect, but more score
                hw_new_population[count] = rand_gene
                count += 1
                pbar.update(1)
        pbar.close()
        '''
        ...
        for l in range(num_layers):
            ...
            while count < num_pop:
                ...
                reward, constraint = self.exterior_search(0, self.model_defs[l], hw_new_population[count], rand_gene)
                if not(reward==None and (constraint==None or constraint > self.constraint_value)): #select valid gene. Incorrect, but more score
                    map_new_population[l][count] = rand_gene
                    count += 1
                    pbar.update(1)
        pbar.close()
    ...
    def genetic_search(self, epochs=100, chkpt_file="genetic_chkpt.plt",fd=None):
        ...
        import pickle
        initial_pop_filename = "initalpop_optuna.bin"
        ...
        def objective(trial):
            x = []
            x.append(trial.suggest_float("L2", 0.0, 1.0)) #       None,
            x.append(trial.suggest_float("L1", 0.0, 1.0)) #        None,
            x.append(trial.suggest_int("PE", MIN_PE, MAX_PE)) #       range(MIN_PE, MAX_PE+1),#PE
            x.append(1) #        range(1, 1000+1), #BW
            x.append(2) #        [2], #NumDim
            x.append(trial.suggest_float("DIMS0", 0.0, 1.0)) #        None, #DIMSize0
            x.append(trial.suggest_float("DIMS1", 0.0, 1.0)) #        None,
            x.append(trial.suggest_float("DIMS2", 0.0, 1.0)) #        None,
            x.append(trial.suggest_int("ParDimK", 1, 6)) #        range(1,6+1), #ParDimK
            x.append(trial.suggest_int("ParDimC", 1, 6)) #        range(1,6+1),
            x.append(trial.suggest_int("ParDimY", 1, 6)) #        range(1,6+1),
            x.append(trial.suggest_int("ParDimX", 1, 6)) #        range(1,6+1),
            x.append(trial.suggest_int("ParDimR", 1, 6)) #        range(1,6+1),
            x.append(trial.suggest_int("ParDimS", 1, 6)) #        range(1,6+1),
            x.append(trial.suggest_int("dens", 0, MAX_GROUP)) #        range(0, MAX_GROUP+1), #group density
            x.append(trial.suggest_int("bank", MIN_BANK, 4)) #        range(MIN_BANK, 4+1), #bank
            x.append(trial.suggest_int("L2buf_w", 100, 4*128000)) #        range(100, 4*128000+1), #L2Buf_weight
            x.append(trial.suggest_int("L2buf_i", 100, 4*128000)) #        range(100, 4*128000+1),
            x.append(trial.suggest_int("L2buf_o", 100, 4*128000)) #        range(100, 4*128000+1),
            x.append(trial.suggest_float("L1buf_w", 0.0, 1.0)) #        None, #L1Buf_weight
            x.append(trial.suggest_float("L1buf_i", 0.0, 1.0)) #        None,
            x.append(trial.suggest_float("L1buf_o", 0.0, 1.0)) #        None,
            for l in range(num_layers):
                x.append(trial.suggest_int("L2OR_K", 1, 6)) #    range(1,6+1), #ARR_LOOP_ORDER_K
                x.append(trial.suggest_int("L2OR_C", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L2OR_Y", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L2OR_X", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L2OR_R", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L2OR_S", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_float("L2TS_K", 0.0, 1.0)) #    None, #ARR_TILE_SIZE_K
                x.append(trial.suggest_float("L2TS_C", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L2TS_Y", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L2TS_X", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L2TS_R", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L2TS_S", 0.0, 1.0)) #    None,
                x.append(trial.suggest_int("L1OR_K", 1, 6)) #    range(1,6+1), #PE_LOOP_ORDER_K
                x.append(trial.suggest_int("L1OR_C", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L1OR_Y", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L1OR_X", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L1OR_R", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_int("L1OR_S", 1, 6)) #    range(1,6+1),
                x.append(trial.suggest_float("L1TS_K", 0.0, 1.0)) #   None, #L1_TILE_SIZE_K
                x.append(trial.suggest_float("L1TS_C", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L1TS_Y", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L1TS_X", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L1TS_R", 0.0, 1.0)) #    None,
                x.append(trial.suggest_float("L1TS_S", 0.0, 1.0)) #    None,
        ...
        print(source_solutions[0])
        for sol in source_solutions:
            trial = {}
            trial["L2"] = sol[0] # x.append(trial.suggest_float("L2", 0.0, 1.0)) #       None,
            trial["L1"] = sol[1] # x.append(trial.suggest_float("L1", 0.0, 1.0)) #        None,
            trial["PE"] = sol[2] # x.append(trial.suggest_int("PE", MIN_PE, MAX_PE)) #       range(MIN_PE, MAX_PE+1),#PE
            #trial["L2"] = sol[3] # x.append(1) #        range(1, 1000+1), #BW
            #trial["L2"] = sol[4] # x.append(2) #        [2], #NumDim
            trial["DIMS0"] = sol[5] # x.append(trial.suggest_float("DIMS0", 0.0, 1.0)) #        None, #DIMSize0
            trial["DIMS1"] = sol[6] # x.append(trial.suggest_float("DIMS1", 0.0, 1.0)) #        None,
            trial["DIMS2"] = sol[7] # x.append(trial.suggest_float("DIMS2", 0.0, 1.0)) #        None,
            trial["ParDimK"] = sol[8] # x.append(trial.suggest_int("ParDimK", 1, 6)) #        range(1,6+1), #ParDimK
            trial["ParDimC"] = sol[9] # x.append(trial.suggest_int("ParDimC", 1, 6)) #        range(1,6+1),
            trial["ParDimY"] = sol[10] # x.append(trial.suggest_int("ParDimY", 1, 6)) #        range(1,6+1),
            trial["ParDimX"] = sol[11] # x.append(trial.suggest_int("ParDimX", 1, 6)) #        range(1,6+1),
            trial["ParDimR"] = sol[12] # x.append(trial.suggest_int("ParDimR", 1, 6)) #        range(1,6+1),
            trial["ParDimS"] = sol[13] # x.append(trial.suggest_int("ParDimS", 1, 6)) #        range(1,6+1),
            trial["dens"] = sol[14] # x.append(trial.suggest_int("dens", 0, MAX_GROUP)) #        range(0, MAX_GROUP+1), #group density
            trial["bank"] = sol[15] # x.append(trial.suggest_int("bank", MIN_BANK, 4)) #        range(MIN_BANK, 4+1), #bank
            trial["L2buf_w"] = sol[16] # x.append(trial.suggest_int("L2buf_w", 100, 4*128000)) #        range(100, 4*128000+1), #L2Buf_weight
            trial["L2buf_i"] = sol[17] # x.append(trial.suggest_int("L2buf_i", 100, 4*128000)) #        range(100, 4*128000+1),
            trial["L2buf_o"] = sol[18] # x.append(trial.suggest_int("L2buf_o", 100, 4*128000)) #        range(100, 4*128000+1),
            trial["L1buf_w"] = sol[19] # x.append(trial.suggest_float("L1buf_w", 0.0, 1.0)) #        None, #L1Buf_weight
            trial["L1buf_i"] = sol[20] # x.append(trial.suggest_float("L1buf_i", 0.0, 1.0)) #        None,
            trial["L1buf_o"] = sol[21] # x.append(trial.suggest_float("L1buf_o", 0.0, 1.0)) #        None,
            for l in range(num_layers):
                trial["L2OR_K"] = sol[22+l*24+0] # x.append(trial.suggest_int("L2OR_K", 1, 6)) #    range(1,6+1), #ARR_LOOP_ORDER_K
                trial["L2OR_C"] = sol[22+l*24+1] # x.append(trial.suggest_int("L2OR_C", 1, 6)) #    range(1,6+1),
                trial["L2OR_Y"] = sol[22+l*24+2] # x.append(trial.suggest_int("L2OR_Y", 1, 6)) #    range(1,6+1),
                trial["L2OR_X"] = sol[22+l*24+3] # x.append(trial.suggest_int("L2OR_X", 1, 6)) #    range(1,6+1),
                trial["L2OR_R"] = sol[22+l*24+4] # x.append(trial.suggest_int("L2OR_R", 1, 6)) #    range(1,6+1),
                trial["L2OR_S"] = sol[22+l*24+5] # x.append(trial.suggest_int("L2OR_S", 1, 6)) #    range(1,6+1),
                trial["L2TS_K"] = sol[22+l*24+6] # x.append(trial.suggest_float("L2TS_K", 0.0, 1.0)) #    None, #ARR_TILE_SIZE_K
                trial["L2TS_C"] = sol[22+l*24+7] # x.append(trial.suggest_float("L2TS_C", 0.0, 1.0)) #    None,
                trial["L2TS_Y"] = sol[22+l*24+8] # x.append(trial.suggest_float("L2TS_Y", 0.0, 1.0)) #    None,
                trial["L2TS_X"] = sol[22+l*24+9] # x.append(trial.suggest_float("L2TS_X", 0.0, 1.0)) #    None,
                trial["L2TS_R"] = sol[22+l*24+10] # x.append(trial.suggest_float("L2TS_R", 0.0, 1.0)) #    None,
                trial["L2TS_S"] = sol[22+l*24+11] # x.append(trial.suggest_float("L2TS_S", 0.0, 1.0)) #    None,
                trial["L1OR_K"] = sol[22+l*24+12] # x.append(trial.suggest_int("L1OR_K", 1, 6)) #    range(1,6+1), #PE_LOOP_ORDER_K
                trial["L1OR_C"] = sol[22+l*24+13] # x.append(trial.suggest_int("L1OR_C", 1, 6)) #    range(1,6+1),
                trial["L1OR_Y"] = sol[22+l*24+14] # x.append(trial.suggest_int("L1OR_Y", 1, 6)) #    range(1,6+1),
                trial["L1OR_X"] = sol[22+l*24+15] # x.append(trial.suggest_int("L1OR_X", 1, 6)) #    range(1,6+1),
                trial["L1OR_R"] = sol[22+l*24+16] # x.append(trial.suggest_int("L1OR_R", 1, 6)) #    range(1,6+1),
                trial["L1OR_S"] = sol[22+l*24+17] # x.append(trial.suggest_int("L1OR_S", 1, 6)) #    range(1,6+1),
                trial["L1TS_K"] = sol[22+l*24+18] # x.append(trial.suggest_float("L1TS_K", 0.0, 1.0)) #   None, #L1_TILE_SIZE_K
                trial["L1TS_C"] = sol[22+l*24+19] # x.append(trial.suggest_float("L1TS_C", 0.0, 1.0)) #    None,
                trial["L1TS_Y"] = sol[22+l*24+20] # x.append(trial.suggest_float("L1TS_Y", 0.0, 1.0)) #    None,
                trial["L1TS_X"] = sol[22+l*24+21] # x.append(trial.suggest_float("L1TS_X", 0.0, 1.0)) #    None,
                trial["L1TS_R"] = sol[22+l*24+22] # x.append(trial.suggest_float("L1TS_R", 0.0, 1.0)) #    None,
                trial["L1TS_S"] = sol[22+l*24+23] # x.append(trial.suggest_float("L1TS_S", 0.0, 1.0)) #    None,

            study.enqueue_trial(trial)


        study.optimize(objective, n_trials=100000000)
