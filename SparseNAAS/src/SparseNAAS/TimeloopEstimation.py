...
class TimeloopEstimator():
    ...
    def write_timeloop_hw(self, dimension, hw_info, mapping_info, filename_=None, layer_id=0):
        ...
        packet_iter = math.ceil((1 * hw_map[DIM.C]) / packet_size)
        ...
        dram_input_bw = (length*packet_size*iteration) / ((length+((length*packet_size/burst_size)*burst_delay)*packet_iter+transaction_delay)*iteration + overhead)

        packet_size = 1 * hw_map[DIM.K] #1byte precision, 
        packet_iter = math.ceil((1 * hw_map[DIM.K]) / packet_size)
        ...
        iteration = 1
        dram_weight_bw = (length*packet_size*iteration) / ((length+((length*packet_size/burst_size)*burst_delay)*packet_iter+transaction_delay)*iteration + overhead)

        packet_size = min(max_bitwidth, 4 * hw_map[DIM.K]) #4byte precision, DIM K
        packet_iter = math.ceil((4 * hw_map[DIM.K]) / packet_size)
        length = (mapping_info.get_mapping_L1_tile_size()[3]*mapping_info.get_mapping_PE_tile_size()[3]) #L1_size_W , for weight stationary
        iteration = (mapping_info.get_mapping_L1_tile_size()[2]*mapping_info.get_mapping_PE_tile_size()[2]) * \
                        (mapping_info.get_mapping_L1_tile_size()[0]*mapping_info.get_mapping_PE_tile_size()[0])/hw_map[DIM.K] #L1_size_H * L1_size_K/hw_K
        dram_output_bw = (length*packet_size*iteration) / ((length+((length*packet_size/burst_size)*burst_delay)*packet_iter+transaction_delay)*iteration + overhead)
