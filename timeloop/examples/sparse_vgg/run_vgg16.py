...
prob_info = []
import csv
with open('prob/vgg16_sparse_group.csv', newline='') as csvfile:
...
def save_prob(result_yaml_prob, example_yaml_prob, layer_info):
    ...
    data['problem']['instance']['densities']['Inputs']['density'] = layer_info[7] # density
...
for i in range(1,14):
    ...
    process.wait()
    print(command, i)
    command = ["cp", "-r", "output_sparse", "output_vgg16/{:02d}".format(i-1)]
    process = Popen(command)
    process.wait()
