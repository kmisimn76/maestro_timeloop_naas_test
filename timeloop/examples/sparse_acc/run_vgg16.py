...
prob_info = []
import csv
with open('prob/vgg16_sparse_cgroup4.csv', newline='') as csvfile:
...
def save_prob(result_yaml_prob, example_yaml_prob, layer_info):
    ...
    data['problem']['instance']['C'] = int(layer_info[1])
    ...
    data['problem']['instance']['densities']['Inputs']['density'] = layer_info[6] # density
...
def save_map(result_yaml_map, example_yaml_map, layer_info):
    ...
    L1_TILENUM_H = 8 if H/14>=8 else (4 if H/14>=4 else (2 if H/14>=2 else 1))
    L1_TILENUM_W = 8 if W/14>=8 else (4 if W/14>=4 else (2 if W/14>=2 else 1))
    L1_TILENUM_R = 3
    L1_TILENUM_S = 3
...
for i in range(1,14):
    ...
    process.wait()
    print(command, i)
    command = ["cp", "-r", "output_sparse", "output_vgg16/{}".format(i-1)]
    process = Popen(command)
    process.wait()
